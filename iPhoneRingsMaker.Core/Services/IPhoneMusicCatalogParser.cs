...
public sealed class IPhoneMusicCatalogParser
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var columns = await IPhoneMusicCatalogSchema.ReadColumnsAsync(connection, "item_extra", cancellationToken);
        var tables = await IPhoneMusicCatalogSchema.ReadTablesAsync(connection, cancellationToken);
        ...
        var locationColumn = IPhoneMusicCatalogSchema.PickColumn(columns, "file", "location", "file_path", "path");
        var formatColumn = IPhoneMusicCatalogSchema.PickColumn(columns, "file_format", "format");
        var durationColumn = IPhoneMusicCatalogSchema.PickColumn(columns, "total_time_ms", "total_time");
        var fileSizeColumn = IPhoneMusicCatalogSchema.PickColumn(columns, "file_size", "size");
        var protectedColumn = IPhoneMusicCatalogSchema.PickColumn(columns, "is_protected", "protected_content", "drm_version_number");
        var artistColumn = IPhoneMusicCatalogSchema.PickColumn(columns, "artist", "item_artist");
        var albumColumn = IPhoneMusicCatalogSchema.PickColumn(columns, "album", "album_name");
        var artworkIdentifierColumn = IPhoneMusicCatalogSchema.PickColumn(
            columns,
            "artwork_cache_id",
            "artwork_id",
            "artwork_token");
        var artworkUrlColumn = IPhoneMusicCatalogSchema.PickColumn(columns, "artwork_url", "artworkURL");
        var artworkDataColumn = IPhoneMusicCatalogSchema.PickColumn(columns, "artwork_data", "artwork");
        var itemArtistColumns = tables.Contains("item_artist")
            ? await IPhoneMusicCatalogSchema.ReadColumnsAsync(connection, "item_artist", cancellationToken)
            : [];
        var baseLocationColumns = tables.Contains("base_location")
            ? await IPhoneMusicCatalogSchema.ReadColumnsAsync(connection, "base_location", cancellationToken)
            : [];
        var itemStoreColumns = tables.Contains("item_store")
            ? await IPhoneMusicCatalogSchema.ReadColumnsAsync(connection, "item_store", cancellationToken)
            : [];
        var albumColumns = tables.Contains("album")
            ? await IPhoneMusicCatalogSchema.ReadColumnsAsync(connection, "album", cancellationToken)
            : [];
        var normalizedArtistColumn = IPhoneMusicCatalogSchema.PickColumn(itemArtistColumns, "artist", "item_artist", "name");
        var normalizedAlbumColumn = IPhoneMusicCatalogSchema.PickColumn(albumColumns, "album", "title", "name");
        var basePathColumn = IPhoneMusicCatalogSchema.PickColumn(baseLocationColumns, "path", "base_path", "location");
        var storeProtectedColumn = IPhoneMusicCatalogSchema.PickColumn(itemStoreColumns, "is_protected");
        ...
        var artistSelect = IPhoneMusicCatalogSchema.SelectOrNull(artistColumn, "e") ??
            (canJoinArtist
                ? $"ia.\"{normalizedArtistColumn}\""
                : "NULL");
        var albumSelect = IPhoneMusicCatalogSchema.SelectOrNull(albumColumn, "e") ??
            (canJoinAlbum
        ...
        command.CommandText = $"""
            SELECT
                e.item_pid,
                e.title,
                {IPhoneMusicCatalogSchema.SelectOrNull(locationColumn)} AS location,
                {IPhoneMusicCatalogSchema.SelectOrNull(formatColumn)} AS file_format,
                {IPhoneMusicCatalogSchema.SelectOrNull(durationColumn)} AS total_time,
                {IPhoneMusicCatalogSchema.SelectOrNull(fileSizeColumn)} AS file_size,
                ...
                {IPhoneMusicCatalogSchema.SelectOrNull(artworkDataColumn)} AS artwork_data,
